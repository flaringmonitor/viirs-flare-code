...
SELECT 
    ...
    SUM(sq.rh) as sum_rh,
    ARRAY_AGG(sq.rh ORDER BY sq.rh) AS list_rh
FROM
(
    SELECT 
        ST_ClusterDBSCAN(flares.point,.00135,1) OVER (PARTITION BY EXTRACT(YEAR FROM (flares.date_ltz)),EXTRACT(WEEK FROM (flares.date_ltz))) AS cid,
        flares.point,
        flares.rh,
        flares.temp_bb,
        flares.date_ltz
